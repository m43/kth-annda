...
def add_noise(pattern, proportion):
    noisy = deepcopy(pattern)
    nflips = int(proportion * pattern.shape[0])
    # print(nflips)
    indexes = np.random.choice(pattern.shape[0], nflips) # sample random indexes
    for x in indexes:
        noisy[x] = -1 if noisy[x] == 1 else 1
    # noisy[indexes] *= -1 # flip sign
    print(proportion, len(indexes), " - ", np.sum(noisy != pattern))
    # return noisy
