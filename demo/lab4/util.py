...
mnist_test_images_filename = "t10k-images-idx3-ubyte"
mnist_test_labels_filename = "t10k-labels-idx1-ubyte"
...
def read_mnist(mnist_dataset_location, dim=[28, 28], n_train=60000, n_test=1000):
    """
    Read mnist train and test data. Images are normalized to be in range [0,1]. Labels are one-hot coded.
    """

    train_imgs = load_idxfile(os.path.join(mnist_dataset_location, mnist_train_images_filename))
    train_imgs = train_imgs / 255.
    train_imgs = train_imgs.reshape(-1, dim[0] * dim[1])

    train_lbls = load_idxfile(os.path.join(mnist_dataset_location, mnist_train_labels_filename))
    train_lbls_1hot = np.zeros((len(train_lbls), 10), dtype=np.float32)
    train_lbls_1hot[range(len(train_lbls)), train_lbls] = 1.

    test_imgs = load_idxfile(os.path.join(mnist_dataset_location, mnist_test_images_filename))
    test_imgs = test_imgs / 255.
    test_imgs = test_imgs.reshape(-1, dim[0] * dim[1])

    test_lbls = load_idxfile(os.path.join(mnist_dataset_location, mnist_test_labels_filename))
