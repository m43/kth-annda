...
sine_mae = statistics.mean(
    [math.fabs(sine_outputs[i] - test_sine_targets[i]) for i in range(len(sine_targets))])
...
plt.plot(inputs, test_sine_targets, label='true values')
...
square_mae = statistics.mean(
    [math.fabs(square_outputs[i] - test_square_targets[i]) for i in range(len(square_targets))])
corrected_square_mae = statistics.mean(
    [math.fabs(corrected_square_outputs[i] - test_square_targets[i]) for i in range(len(square_targets))])
...
plt.plot(inputs, test_square_targets, label='true values')
